...
const SurveyCard: React.FC<SurveyPageProps> = ({
  cardData,
  setData,
  selectedAnswers,
  onPrev,
  ...
  const [healthStatus, setHealthStatus] = useState(
    selectedAnswers?.healthStatus || ""
  );
  const [reason, setReason] = useState(selectedAnswers?.reason || "");

  useEffect(() => {
    if (selectedAnswers?.ageRange) setAgeRange(selectedAnswers.ageRange);
    if (selectedAnswers?.genderPreference)
      setGenderPreference(selectedAnswers.genderPreference);
    if (selectedAnswers?.healthStatus)
      setHealthStatus(selectedAnswers.healthStatus);
        ...
        <div className="survey-card__options">
          <PRadioButton
            ...
            onSelectionChange={(data) =>
              setSelectionData(cardData.questionField, data)
            }
            selectedOption={
              selectedAnswers?.[
                cardData.questionField as keyof typeof selectedAnswers
              ] as string
            }
          />
        </div>
